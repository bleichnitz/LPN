...
def value_conversion(val, conversion_type):
    ...
    if conversion_type == "letter_to_word" or conversion_type == "letter_to_number":
        for l in letters:
            if l[search_index] == val:
                return l[return_index]
    elif conversion_type == "number_to_word":
        if val == 0:
            return "Incomplete"
...
def achievement_frequency(filter_type, olg_codes, sc_codes):
    ...
    print("")
    print("\t\tEnter the number of times student needs to demonstrate the following criteria at the PROFICIENCY level to "
          "earn the credit (frequency should be >= 1):")
    for index in range (0, num_categories):
        ...
        if frequency <= 1:
            cat_frequency.append(1)           # Comprehehsive
            cat_frequency.append(1)           # Exemplary
        else:
            cat_frequency.append(frequency-1)     # Comprehehsive
...
def evaluate_achievements(num_students, student_list, num_criteria, data_to_analyze):
    ...
    return {"Criteria Achievement":class_criteria_achievement, "Cascading Achievement":class_cascading_achievement}
...
def determine_high_achievements(num_students, student_list, data_to_analyze, data_for_activity_count, frequency_requirements):
    # data_to_analyze is now organized by student [{},{},{}....]
    print(f"\t\t\tCategory High Achievements:")
    number_of_activities_in_categories = []
    for category in range (0, len(data_for_activity_count)):
        num_activities = len(data_for_activity_count[category])
        number_of_activities_in_categories.append(num_activities)
    ...
    for student in range(0, len(data_to_analyze)):
         ...
         for category in range(0, len(data_to_analyze[student])):
             ...
             high_achievement = analyze_achievement_dictionary(dictionary=category_dictionary,
                                                               category_frequency=category_frequencies,
                                                               activity_count=number_of_activities_in_categories[category])
             student_high_achievements.append(high_achievement)
         print(f"\t\t\t\t{student_ID} >>> {student_high_achievements}")
         class_high_achievements.append(student_high_achievements)
    return class_high_achievements


def analyze_achievement_dictionary(dictionary, category_frequency, activity_count):
    """
    This function takes in a SINGLE high achievement from a student and 'calibrates it based on (a) the percentage
    of success - e.g. if you have a significant number of incompletes, the category will be changed to incomplete - or
    (b) you set th category achievement frequency to >1, you can start to get credit towards the next step on the
    learning progression.
    :param dictionary:
    :param category_frequency:
