...
def create_graph(filter_type, targets, achievement, goal_labels, f_name, l_name, student_num, save_path):
    ...
    ax.axhspan(0, 0.67, color="red", alpha=0.15)  # this creates the pass threshold zone in the graph
    ...
    if differential < 0.15:
        ax.axhspan(min_grade, max_grade, color="green", alpha=0.25)
    data1 = ax.bar(x - width / 2, included_targets, width, label="Learning Targets", color="grey")
    data2 = ax.bar(x + width / 2, achievement, width, label="Student Achievement", color="blue")
    ...
    ax.set_ylabel("Levels", weight="bold", fontsize=13)
